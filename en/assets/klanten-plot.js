// Customer quadrant at customer level. Aggregated per customer across all
// projects in 2024 + 2025. Color follows sub-sector. Bubble size =
// number of projects. Hover link between point and table.
...
(function () {
  ...
  function aggregeer(projecten, subsectorMap) {
    ...
    projecten.forEach(function (p) {
      if (!perKlant[p.klant]) {
        perKlant[p.klant] = {
          klant: p.klant,
          subsector: subsectorMap[p.klant] || "Unknown",
          omzet: 0,
          margeMass: 0,
          projectCount: 0,
          totaalUren: 0,
  ...
  function render(opts) {
    ...
    var svg = el("svg", {
      ...
      viewBox: "0 0 " + W + " " + H,
      role: "img",
      "aria-label": "Customers by revenue versus margin, color per sub-sector",
    });
    ...
    label("large & healthy",    iw - 8, 14, "end");
    label("small & healthy",    8,      14, "start");
    label("large & thin margin", iw - 8, ih - 8, "end");
    label("small & thin margin", 8,      ih - 8, "start");
    ...
    }, "Total revenue per customer over 2024 + 2025"));
